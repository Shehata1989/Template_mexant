...
function HeaderBar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // عند النزول أول مرة 100px → يختفي
      if (
      ...
      // عند تخطي طول الشاشة → يظهر بشكل دائم
      if (currentScrollY >= screenHeight && !isBeyondScreen) {
        setIsVisible(true);
        setIsBeyondScreen(true);
        return;
      }

      // عند العودة داخل طول الشاشة → يختفي مجدداً
      if (currentScrollY < screenHeight && isBeyondScreen) {
        setIsVisible(false);
        setIsBeyondScreen(false);
      }

      // إذا عاد المستخدم للأعلى إلى 0px → استعادة الحالة الأصلية
      if (currentScrollY === 0) {
        setIsVisible(true);
  ...
  }, [hasScrolledOnce, isBeyondScreen]);
  ...
  return (
    <AppBar
      key={isVisible}
      className={`!fixed !shadow-none transition-transform duration-300 ${
